...
static void LCD429_ConfigLTDC(void)
{
	...
	*/
	...
	switch (g_LcdType)
	{
		...
		case LCD_50_800X480:		/* 5.0寸 480 * 272 */
			RCC_PLLSAIConfig(429, 2,  4);
			RCC_LTDCCLKDivConfig(RCC_PLLSAIDivR_Div8);
		...
		case LCD_70_1024X600:		/* 7.0寸 1024 * 600 */
			...
			LTDC_InitStruct.LTDC_VSPolarity = LTDC_VSPolarity_AL;	/* VSYNC 低电平有效 */
			/* Initialize the data enable polarity as active low */
			LTDC_InitStruct.LTDC_DEPolarity = LTDC_DEPolarity_AL;	/* DE 高电平有效 */
			/* Initialize the pixel clock polarity as input pixel clock */
			LTDC_InitStruct.LTDC_PCPolarity = LTDC_PCPolarity_IPC;		// LTDC_PCPolarity_IIPC
		
			Width = 1024;
			Height = 600;

			HSYNC_W = 70;	/* =10时，显示错位，20时部分屏可以的,80时全部OK */
			HBP = 160;
			HFP = 160;
			VSYNC_W = 10;
			VBP = 23;
	...
	//LCD429_LayerInit();  展开此函数
	{
		...
		*/
		LTDC_Layer_InitStruct.LTDC_CFBLineLength = ((g_LcdWidth * 2) + 3);
		/* the pitch is the increment from the start of one line of pixels to the
		start of the next line in bytes, then :
		Pitch = Active high width x number of bytes per pixel
		*/
		LTDC_Layer_InitStruct.LTDC_CFBPitch = (g_LcdHeight * 2);

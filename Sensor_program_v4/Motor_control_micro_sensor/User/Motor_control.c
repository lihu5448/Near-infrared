...
//允许误差
#define WINDOW_DEG_NUM     2u     /* 0.2 = 2/10 */
#define WINDOW_DEG_DEN     10u
#define WINDOW_TICKS   ( (int32_t)((ENC_CPR * WINDOW_DEG_NUM + (360u * WINDOW_DEG_DEN / 2u)) / (360u * WINDOW_DEG_DEN)) )
/* WINDOW_TICKS 约等于 18 */

//编码器稳定次数
const uint8_t STABLE_N = 2u;
uint8_t stable = 0;
...
/* 计算 encoder 误差（最短环绕距离），返回范围约 [-16384, +16384] */
static int32_t enc_err_wrap(int32_t enc_now, int32_t enc_target)
{
    int32_t err = enc_now - enc_target;

    /* wrap到[-ENC_CPR/2, +ENC_CPR/2] */
    if (err >  (int32_t)(ENC_CPR / 2u)) err -= (int32_t)ENC_CPR;
    if (err < -(int32_t)(ENC_CPR / 2u)) err += (int32_t)ENC_CPR;

    return err;
}

/* 由“目标度数”计算“目标encoder tick”（四舍五入） */
static uint16_t deg_to_target_enc(uint16_t target_deg)
{
    /* target_enc = round(target_deg * ENC_CPR / 360) */
    uint32_t tmp = (uint32_t)target_deg * ENC_CPR + 180u; /* +180用于/360四舍五入 */
    return (uint16_t)(tmp / 360u); /* 0..32767 */
}

...
static void FMC_NAND_Init(void)
{
	...
	/*--NAND Flash GPIOs 配置  ------
		...
		PG6/FMC_INT2	(本例程用查询方式判忙，此口线作为普通GPIO输入功能使用)
	*/
	...
	*/
	//p.FMC_HiZSetupTime = 0x01;
	p.FMC_HiZSetupTime = 0x1;


	FMC_NANDInitStructure.FMC_Bank = FMC_Bank2_NAND;  					/* 定义FSMC NAND BANK 号 */
	FMC_NANDInitStructure.FMC_Waitfeature = FMC_Waitfeature_Disable;		/* 插入等待时序使能 */
	FMC_NANDInitStructure.FMC_MemoryDataWidth = FMC_NORSRAM_MemoryDataWidth_8b;	/* 数据宽度 8bit */
	FMC_NANDInitStructure.FMC_ECC = FMC_ECC_Enable; 						/* ECC错误检查和纠正功能使能 */
	FMC_NANDInitStructure.FMC_ECCPageSize = FMC_ECCPageSize_2048Bytes;	/* ECC 页面大小 */
	FMC_NANDInitStructure.FMC_TCLRSetupTime = 0x01;						/* CLE低和RE低之间的延迟，HCLK周期数 */
	FMC_NANDInitStructure.FMC_TARSetupTime = 0x01;						/* ALE低和RE低之间的延迟，HCLK周期数 */
	FMC_NANDInitStructure.FMC_CommonSpaceTimingStruct = &p;				/* FSMC Common Space Timing */
	FMC_NANDInitStructure.FMC_AttributeSpaceTimingStruct = &p;			/* FSMC Attribute Space Timing */
...
static uint8_t FMC_NAND_PageCopyBack(uint32_t _ulSrcPageNo, uint32_t _ulTarPageNo)
{
	...
	{
		nand_printf("Error : FMC_NAND_PageCopyBackEx(src=%d, tar=%d) \r\n", _ulSrcPageNo, _ulTarPageNo);
		return NAND_FAIL;
	}
	...
	*/
	...
	nand_printf("Error: FMC_NAND_PageCopyBack(%d, %d)\r\n", _ulSrcPageNo, _ulTarPageNo);
	return NAND_FAIL;
}
...
*********************************************************************************************************
*/
static uint8_t FMC_NAND_PageCopyBackEx(uint32_t _ulSrcPageNo, uint32_t _ulTarPageNo, uint8_t *_pBuf, uint16_t _usOffset, uint16_t _usSize)
{
	uint16_t i;

	if ((_ulSrcPageNo & 0x40) != (_ulTarPageNo & 0x40))
	{
		nand_printf("Error A18 not same:  FMC_NAND_PageCopyBackEx(src=%d, tar=%d) \r\n", _ulSrcPageNo, _ulTarPageNo);
		return NAND_FAIL;
	}
	...
	*/
	...
	nand_printf("Error: FMC_NAND_PageCopyBackEx(src=%d, tar=%d, offset=%d, size=%d)\r\n",
...
static uint8_t FMC_NAND_WritePage(uint8_t *_pBuffer, uint32_t _ulPageNo, uint16_t _usAddrInPage, uint16_t _usByteCount)
{
	...
	*/
	...
	nand_printf("Error: FMC_NAND_WritePage(page=%d, addr=%d, count=%d)\r\n",
...
*********************************************************************************************************
*/
static uint8_t FMC_NAND_EraseBlock(uint32_t _ulBlockNo)
{
	/* HY27UF081G2A  (128MB)
				  Bit7 Bit6 Bit5 Bit4 Bit3 Bit2 Bit1 Bit0
		第1字节： A7   A6   A5   A4   A3   A2   A1   A0		(_usPageAddr 的bit7 - bit0)
...
uint8_t NAND_Init(void)
{
	...
	FMC_NAND_Init();			/* 配置FSMC和GPIO用于NAND Flash接口 */
...
uint8_t NAND_WriteToNewBlock(uint32_t _ulPhyPageNo, uint8_t *_pWriteBuf, uint16_t _usOffset, uint16_t _usSize)
{
	...
	for (n = 0; n < 10; n++)
	{
		...
		{
			nand_printf("Error: NAND_FindFreeBlock() %d\r\n", usNewBlock);
		...
		if (i == NAND_BLOCK_SIZE)
		{
			...
			{
				nand_printf("Error: FMC_NAND_EraseBlock()\r\n");
...
uint8_t NAND_WriteMultiSectors(uint8_t *_pBuf, uint32_t _SectorNo, uint16_t _SectorSize, uint32_t _SectorCount)
{
	...
	*/
	...
	for (i = 0; i < _SectorCount; i++)
	{
		...
		if (usPBN >= NAND_BLOCK_COUNT)
		{
			nand_printf("Error: NAND_WriteMultiSectors(), no format. usLBN=%d, usPBN=%d\r\n", usLBN, usPBN);
			/* 没有格式化，usPBN = 0xFFFF */
			return NAND_FAIL;
		}
		...
		/* 读出扇区的内容，判断是否全FF */
		if (FMC_NAND_ReadData(s_ucTempBuf, uiPhyPageNo, usAddrInPage, _SectorSize) == NAND_FAIL)
		{
			nand_printf("Error: NAND_WriteMultiSectors(), not 0xFF\r\n");
			return NAND_FAIL;	/* 失败 */
		}
		...
		if (NAND_IsBufOk(s_ucTempBuf, _SectorSize, 0xFF))
		{
			...
			{
				nand_printf("Error: NAND_WriteMultiSectors(), Write Faile\r\n");
				/* 将数据写入到另外一个块（空闲块） */
...
uint8_t NAND_ReadMultiSectors(uint8_t *_pBuf, uint32_t _SectorNo, uint16_t _SectorSize, uint32_t _SectorCount)
{
	...
	*/
	...
	for (i = 0; i < _SectorCount; i++)
	{
		...
		if (usPBN >= NAND_BLOCK_COUNT)
		{
			nand_printf("Error: NAND_ReadMultiSectors(), not format, usPBN = %d\r\n", usPBN);
			/* 没有格式化，usPBN = 0xFFFF */
			return NAND_FAIL;
		}
		...
		{
			nand_printf("Error: NAND_ReadMultiSectors(), ReadData(page = %d, addr = %d)\r\n", uiPhyPageNo, usAddrInPage);
			return NAND_FAIL;	/* 读NAND Flash失败 */
		}
	}
